class php::extension::redis {
  ...
  exec { 'phpize-5.5.18 redis':
    command => '/phpfarm/inst/bin/phpize-5.5.18',
    cwd => '/tmp/redis-2.2.5',
    require => Exec['tar xzf redis-2.2.5.tgz']
  }

  exec { '/bin/su - root -mc "cd /tmp/redis-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.5.18 --enable-redis-igbinary"':
    timeout => 0,
    require => Exec['phpize-5.5.18 redis']
  }

  exec { '/bin/su - root -mc "cd /tmp/redis-2.2.5 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/redis-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.5.18 --enable-redis-igbinary"']
  }
