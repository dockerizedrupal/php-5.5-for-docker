class php::extension::redis {
  ...
  exec { '/bin/bash -l -c "cd /tmp/redis-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.5.18 --enable-redis-igbinary"':
    timeout => 0,
  ...
  exec { '/bin/bash -l -c "cd /tmp/redis-2.2.5 && make"':
    timeout => 0,
    require => Exec['/bin/bash -l -c "cd /tmp/redis-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.5.18 --enable-redis-igbinary"']
  }
