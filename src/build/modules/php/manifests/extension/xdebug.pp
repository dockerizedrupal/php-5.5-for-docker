class php::extension::xdebug {
  ...
  exec { '/bin/bash -l -c "cd /tmp/xdebug-2.2.6 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.5.18"':
    timeout => 0,
  ...
  exec { '/bin/bash -l -c "cd /tmp/xdebug-2.2.6 && make"':
    timeout => 0,
    require => Exec['/bin/bash -l -c "cd /tmp/xdebug-2.2.6 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.5.18"']
  }
